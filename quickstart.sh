...
echo ""
# ==========================================
# Opencode Setup (Auto-Generated)
# ==========================================
if command -v opencode &> /dev/null; then
    echo "✨ Opencode detected! Setting up Hive integration..."
    
    # Create directory structure
    mkdir -p .opencode/agents
    
    # Determine OS for correct path separator (Windows uses ';', Mac/Linux uses ':')
    ...
    # 1. Generate mcp.json with correct separator
...
EOF
    ...
    # 2. Generate Native Agent (hive.md)
    cat > .opencode/agents/hive.md <<EOF
---
name: hive
...
- Always use the \`agent-builder\` MCP server for filesystem operations related to agents.
EOF

    echo "✅ Opencode configuration created in .opencode/"
else
    echo "ℹ️  Opencode not found. Skipping setup."
fi
# Opencode Setup
...
# ==========================================
if command -v python &> /dev/null; then
    echo "🐍 Python detected. Running Opencode setup..."
    python setup_opencode.py
elif command -v python3 &> /dev/null; then
    echo "🐍 Python3 detected. Running Opencode setup..."
    python3 setup_opencode.py
else
    echo "⚠️ Python not found. Skipping Opencode setup."
fi